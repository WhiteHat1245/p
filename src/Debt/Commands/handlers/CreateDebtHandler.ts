...
async execute(command: CreateDebtCommand): Promise<Debt> {
    ...
    const currentTotalDebt = await this.queryBus.execute(new GetTotalDebtQuery(createDebtDto.customerId));
    const newTotalDebt = currentTotalDebt + createDebtDto.amount;

    if (newTotalDebt > DEBT_LIMIT) {
      throw new BadRequestException(`Creating this debt would exceed the customer's limit of ${DEBT_LIMIT}. Current debt: ${currentTotalDebt}`);
    }
