...
export class UpdateDebtHandler implements ICommandHandler<UpdateDebtCommand> {
  ...
  async execute(command: UpdateDebtCommand): Promise<Debt> {
    ...
    const currentTotalDebt = await this.queryBus.execute(
      new GetTotalDebtQuery(debt.CustomerID),
    );
    const newTotalDebt = currentTotalDebt - debt.Amount + newAmount;

    if (newTotalDebt > DEBT_LIMIT) {
      throw new BadRequestException(
        `Updating this debt would exceed the customer's limit of ${DEBT_LIMIT}. Current debt: ${currentTotalDebt - debt.Amount}`,
      );
    }

    const updatedDebt = await this.debtRepository.preload({
      DebtID: id,
      ...updateDebtDto,
    });
    if (!updatedDebt) {
      throw new NotFoundException(
        `Debt with ID ${id} could not be preloaded for update`,
      );
    }
