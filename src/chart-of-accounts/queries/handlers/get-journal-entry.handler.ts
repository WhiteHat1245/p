...
export class GetJournalEntryHandler implements IQueryHandler<GetJournalEntryQuery> {
    ...
    async execute(query: GetJournalEntryQuery): Promise<JournalEntry> {
        return this.journalEntryRepository.findOneOrFail({ where: { id: query.id }, relations: ['details'] });
    }
}