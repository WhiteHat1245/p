...
export class GetEggInventoryHandler implements IQueryHandler<GetEggInventoryQuery> {
  constructor(
  ...
  async execute(query: GetEggInventoryQuery): Promise<EggInventory> {
    ...
    const inventory = await this.eggInventoryRepository.findOne({ where: { InventoryID: inventoryId } });
    if (!inventory) {
      throw new NotFoundException(`Egg Inventory with ID ${inventoryId} not found`);
    }
    return inventory;
  }
}
