...
export class GetSlaughterhouseHandler implements IQueryHandler<GetSlaughterhouseQuery> {
  constructor(
  ...
  async execute(query: GetSlaughterhouseQuery): Promise<Slaughterhouse> {
    ...
    const slaughterhouse = await this.slaughterhouseRepository.findOne({ where: { SlaughterhouseID: slaughterhouseId }, relations: ['PoultryBatches', 'FrozenPoultryInventories'] });
    if (!slaughterhouse) {
      throw new NotFoundException(`Slaughterhouse with ID ${slaughterhouseId} not found`);
    }
